...
public class RiskCreditUser {
    ...
    /***********************新增同盾信息********************************/
    private Date tdVerifyTime;
    private Date tdVerifyNextTime;
     ...
     *
     * @param userId         用户主键
     * @param zmIndustyBlack 行业关注度黑名单1是2否
     * @param zmOver         行业关注度接口中返回的借贷逾期记录数AA001借贷逾期的记录数
     * @param zmNoPayOver    行业关注度接口中返回的逾期未支付记录数，包括AD001 逾期未支付、AE001 逾期未支付的记录总数
     */
     ...
     *
     * @param id                            征信主键
     * @param tdScore                       同盾分，分值越高风险越大
     * @param tdPhoneBlack                  同盾手机网贷黑名单个数
     * @param tdCardNumBlack                同盾身份证号网贷黑名单个数
     * @param tdMonth1Borrow                同盾1月内多平台申请借款
     * @param tdDay7Borrow                  同盾7天内多平台申请借款
     * @param tdMonth1CardNumDeviceBorrow   同盾1月内身份证使用多设备申请
     * @param tdDay7DeviceCardOrPhoneBorrow 同盾1月内设备使用多身份证或手机号申请
     * @param tdDay7CardDevice              7天内身份证关联设备数
     * @param tdMonth3ApplyCard             3个月内申请信息关联多个身份证
     * @param tdMonth3CardApply             3个月内身份证关联多个申请信息
     ...
     *
     * @param id       征信表主键
     * @param bqsBlack 是否是黑名单 1.通过；2.拒绝（命中黑名单）；3.建议人工审核(命中灰名单)
     */
     ...
     *
     * @param id        主键
     * @param jyLoanNum 借款信息总数
     * @param jyJdNum   拒单数
     * @param jyJdBl    拒单比例
     * @param jyOverNum 逾期数，逾期金额大于0或者状态是逾期
     * @param jyOverBl  逾期比例
     * @param jyFkNum   91征信已放款总数
     * @param jyHkNum   91征信已还款总数
     * @param jyHkBl    91征信还款比例
     */
     ...
     *
     * @param id           征信主键
     * @param mgBlackScore 密罐黑中介分
     * @param mgDay7Num    密罐七天内被查询次数
     * @param mgMonth1Num  密罐一个月内被查询次数
     * @param mgBlack      密罐黑名单 ,1是2否；身份证和姓名是否在黑名单或者手机和姓名是否在黑名单
     */
     ...
     * @param jxlLink1Order   聚信立第二联系人通话次数排名
     * @param jxlGjTs         聚信立关机天数，手机静默情况
     * @param jxlHtPhoneNum   聚信立互通电话数量（结合电话详单和手机通讯录）
     * @param jxlAmthNum      聚信立澳门通话次数
     * @param jxlPhoneRegDays 手机号开户日期到当前时间的天数
     * @param jxlDetail       聚信立详情
     * @param jxlTotal        聚信立关机天数统计周期(天数)
     * @param jxlGjBl         聚信立关机比例
     ...
     *
     * @param id         主键ID
     * @param riskStatus 机审状态
     * @param riskRemark 机审备注
     */
     ...
     *
     * @param id                        主键
     * @param presentAddNum             常用地址重复次数
     * @param companyAddNum             公司地址重复次数
     * @param manyDeviceLoginNum        多设备登录同一个账号次数
     * @param oneDeviceRegManyPhoneNum  一个设备注册多个用户次数
     * @param companyNameNum            单位名称出现次数
     * @param companyPhoneDiffNameOrAdd 同一个单位电话关联不同单位名称或地址的次数
     * @param companyAddDiffNameOrPhone 同一个单位地址关联不同单位号码或者名称
     * @param companyNameDiffAddOrPhone 同一个单位名称关联不同地址或电话
     * @param concatApplyNum            联系人申请借款数
     */
     ...
     * @param userId           用户主键
     * @param yxFxNum          风险记录数
     * @param yxMonth3Over     逾期超过三个月的次数
     * @param yxLessMonth3Over 逾期未超过三个月次数
     * @param yxStatus         宜信接口状态
     */
