...
function againBankCard(){
	    	...
	    	data.id=$("#cardId").val();
			openAjax('${path}/yeepayBindCard/credit-card/validBorrowOrder?deviceId=${deviceId}&mobilePhone=${mobilePhone}', data,resultSave);
}
function resultSave(data){
	if(data.code=="0"){
		window.location.href ="${path}/yeepayBindCard/credit-card/updateUserBank?deviceId=${deviceId}&mobilePhone=${mobilePhone}";
   	}
	else if(data.code=="-2"){
